...
function StatCard({ label, value, sub, accent }: { label:string; value:string; sub?:string; accent?:boolean }) {
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-5">
      ...
      <div className={`text-xl sm:text-2xl font-bold tracking-tight leading-none ${accent?'text-red-600':'text-black'}`}>{value}</div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-28">
      ...
      <header className="border-b border-gray-100 bg-white sticky top-0 z-10 overflow-visible no-print">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-1 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-base sm:text-xl font-semibold text-black">Print Report</h1>
          </div>
          <button onClick={()=>window.print()}
            className="px-3 sm:px-4 py-2 bg-black text-white text-xs sm:text-sm font-medium rounded-xl hover:bg-black/90 transition-colors flex-shrink-0">
            Export PDF
          </button>
        </div>
      </header>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        {/* Job summary */}
        <div className="bg-black rounded-2xl px-4 sm:px-6 py-5 mb-5 sm:mb-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-5 sm:mb-6">
          ...
          <StatCard label="Errors Detected" value={String(r.errorsDetected)}
            sub={r.errorsDetected===0?'No issues':`${r.errorRate} error rate`} accent={r.errorsDetected>0}/>
          <StatCard label="Duration" value={r.duration} sub="wall time"/>
        </div>

        {/* Quality score */}
        <div className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 mb-5 sm:mb-6">
          <div className="flex items-center justify-between mb-3">
            <div className="text-[10px] font-semibold text-black/30 uppercase tracking-widest">Quality Score</div>
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${
              score>=90?'border-black/10 bg-black/5 text-black':score>=75?'border-black/10 bg-black/5 text-black':'border-red-200 bg-red-50 text-red-600'
            }`}>
              {score>=90?'Excellent':score>=75?'Acceptable':'Needs Review'}
            </span>
          </div>
          <div className="flex items-baseline gap-2 mb-3">
            <span className="text-3xl sm:text-4xl font-bold text-black tracking-tight">{score}</span>
            <span className="text-lg text-black/20 font-medium">/ 100</span>
          </div>
          <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full rounded-full bg-black transition-all duration-1000" style={{width:`${score}%`}}/>
          </div>
          <p className="text-[11px] text-black/30 mt-2">{r.errorsDetected} error{r.errorsDetected!==1?'s':''} across {r.totalLayers} layers</p>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
          ...
          {r.alerts.length === 0 ? (
            ...
            <div className="divide-y divide-gray-50">
              ...
              <div className="hidden sm:grid sm:grid-cols-5 px-5 py-2 bg-gray-50">
              ...
              {r.alerts.map((a:any)=>(
                <div key={a.id}>
                  {/* Desktop row */}
                  <div className="hidden sm:grid sm:grid-cols-5 px-5 py-3 items-center">
                    ...
                    <div className="text-xs font-mono font-medium text-black">{a.angle>0?'+':''}{a.angle}°</div>
                  ...
                  <div className="sm:hidden px-4 py-3 space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-black">{a.cameraLabel}</span>
                      <span className="text-xs font-mono font-bold text-black">{a.angle>0?'+':''}{a.angle}°</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] text-black/40 font-mono">
                        {new Date(a.time).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})} · Layer {a.layer}
                      </span>
                      <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full border border-black/10 bg-black/5 text-black">
                        {a.type?.toUpperCase()}
                      </span>
                    </div>
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 z-10 no-print">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <button onClick={()=>router.push('/projects')}
              className="flex-1 sm:flex-none px-4 py-2.5 text-sm border border-gray-200 rounded-xl text-black hover:bg-gray-50 transition-colors">
              Projects
            </button>
            <button onClick={()=>window.print()}
              className="flex-1 sm:flex-none px-4 sm:px-6 py-2.5 text-sm font-semibold bg-black text-white rounded-xl hover:bg-black/90 transition-colors">
              Export PDF
            </button>
          </div>
        </div>
